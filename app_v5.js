...
(function(){
  // avoid stale SW caches on Pages
  ...
  function beep(freq=880,dur=0.08,vol=60){ if(!sound) return; try{ AC=AC||new (window.AudioContext||window.webkitAudioContext)(); const o=AC.createOscillator(); const g=AC.createGain(); o.type='sine'; o.frequency.value=freq; g.gain.value=vol/100; o.connect(g).connect(AC.destination); o.start(); setTimeout(()=>o.stop(), dur*1000);}catch(e){} }
  function flash(t){ const s=$('#status'); s.textContent=t; s.style.background='var(--accent)'; s.style.color='#062016'; setTimeout(()=>{ s.textContent='Ready'; s.style.background='var(--card)'; s.style.color='var(--muted)'; },1400); }
  ...
  const DESC = {
    ...
    "Landmine Press":"Bar anchored; arc press; hips square.",
    "Incline Push-Ups":"Hands elevated; body straight; chest to edge.",
    "Good Morning":"Hinge with bar on back; neutral spine.",
    "DB Romanian Deadlift":"Hinge with DBs; slide along thighs.",
    "DB Swing":"Hip snap swings the DB to chest height.",
    "Hip Thrust":"Upper back on bench; lockout with glutes.",
    "Walking Lunge":"Alternating steps; knee tracks toes.",
    "Reverse Lunge":"Step back; shin vertical; drive through front heel.",
    "Broad Jump":"Two-foot jump for distance; stick landing.",
    "Hollow Hold":"Lower back pressed to floor; long lines.",
    "Row Erg Sprint 20s":"All-out 20s; split low; tall posture."
  };
  ...
  const state = { day: +LS.get('currentDay',0), cursor:0, guided: LS.get('guided', true), timer:null, remaining:0, total:0, paused:false, running:false };
  ...
  function mount(){
    const root=document.createElement('div'); root.className='container'; root.innerHTML = `
      ...
      <footer>Guided mode: “Set done” → auto-rest with chime, then prompt for next set. Smart swaps respect your equipment.</footer>
    `;
    ...
    // Equipment init
    LS.set('equip', LS.get('equip', Object.fromEntries(EQUIP_ALL.map(e=>[e,true]))));
    ...
    on('#pauseBtn','click', ()=>{ if(state.running){ state.paused=true; set('timerLabel','Paused'); } });
    on('#resumeBtn','click', ()=>{ if(state.running && state.paused){ state.paused=false; set('timerLabel','Resumed'); } });
  ...
  function key(s){ return `hybrid_${s}_day${state.day}`; }
  function persistDone(i,v){ const m=LS.get(key('done'),{}); m[i]=v; LS.set(key('done'),m); }
  function persistNote(i,txt){ const m=LS.get(key('notes'),{}); if(txt) m[i]=txt; else delete m[i]; LS.set(key('notes'),m); }

  function renderDay(){
    const list=$('#workoutList'); const day=PLAN.days[state.day];
    list.innerHTML='';
    const doneMap=LS.get(key('done'),{}), notesMap=LS.get(key('notes'),{});

    day.blocks.forEach((b, idx)=>{
      const id=`ex-${idx}`;
      const card=document.createElement('div'); card.className='card'+(state.cursor===idx?' current':'');
      const tips = DESC[b.name] ? `<button class="ghost" data-action="tips" data-i="${idx}">Tips</button>` : '';
      card.innerHTML = `
        <h3>${b.name}</h3>
    ...
    list.onclick = (e)=>{
      ...
      if(btn.dataset.action==='set'){
        ...
        $('#'+id+'-count').value=v; $('#'+id+'-setsDone').textContent=v; persistDone(i,v); beep(880,0.06);

        if(state.guided){ startTimer(parseInt(ex.rest,10)||60,'rest'); set('timerLabel',`Rest ${ex.rest}s — next: ${ex.name}`); }
      ...
      if(btn.dataset.action==='note'){
        const txt=prompt('Add/edit note for this exercise:', notesMap[i]||''); persistNote(i, txt||'');
        $('#'+id+'-note').textContent = txt ? "📝 "+txt : "";
      }
  ...
  function openSwap(i){
    ...
    function isEquipOk(name){
      const key=name.toLowerCase(), rules=[
    ...
    function nstr(s){return (s||"").toLowerCase();}
    function inferCat(n){
      const s=nstr(n);
    ...
    function candidates(){
      ...
      arr.forEach(o=>{
        ...
        if(nstr(o.name)===nstr(ex.name)) o.score -= 2;
      });
    ...
    const {suggested,others}=candidates();
    function draw(q=''){
      list.innerHTML='';
  ...
  function startTimer(seconds,mode){
    ...
    set('timerLabel', mode==='work'?'Work interval running…':`Rest ${seconds}s`); tick(); t.timer=setInterval(tick,1000);
  }
  function tick(){
    if(t.paused) return;
    if(t.remaining<=0){ stopTimer(); beep(660,0.08); setTimeout(()=>beep(880,0.12,90),100); flash('Rest done — start next set'); return; }
    t.remaining--;
    const mm=String(Math.floor(t.remaining/60)).padStart(2,'0'), ss=String(t.remaining%60).padStart(2,'0');
    set('clock',`${mm}:${ss}`); const p=Math.max(0,Math.min(100,100*((t.total-t.remaining)/t.total))); $('#bar').style.width=p+'%';
    if(t.remaining<=3) beep(520+t.remaining*30,0.06);
  }
  function stopTimer(){ if(t.timer) clearInterval(t.timer); t.timer=null; t.running=false; set('clock','00:00'); $('#bar').style.width='0%'; set('timerLabel','Idle'); }
  function resetDay(){ const d=PLAN.days[state.day]; d.blocks.forEach((b,i)=>{ const inp=document.getElementById(`ex-${i}-count`); if(inp){ inp.value=0; set(`ex-${i}-setsDone`,'0'); } }); stopTimer(); flash('Reset'); }

  /* ---------- Settings ---------- */
  function openSettings(){
    const eq=LS.get('equip',defaultEquip); const grid=$('#equipGrid'); grid.innerHTML='';
    for(const name of EQUIP_ALL){ const id='eq-'+name.replace(/[^a-z0-9]/ig,'_'); const row=document.createElement('label'); row.innerHTML=`<input type="checkbox" id="${id}" ${eq[name]?'checked':''}> <span>${name}</span>`; grid.appendChild(row); }
    $('#settingsModal').style.display='flex';
  }
  function saveSettings(){
    const eq={}; for(const name of EQUIP_ALL){ const id='eq-'+name.replace(/[^a-z0-9]/ig,'_'); const el=document.getElementById(id); eq[name]=!!(el&&el.checked); }
    LS.set('equip',eq); $('#settingsModal').style.display='none'; flash('Equipment saved');
  }

  document.addEventListener('DOMContentLoaded', ()=>{
    document.body.innerHTML=''; const c=document.createElement('div'); c.id='app'; document.body.appendChild(c);
    // inject container element and mount UI
    const wrap=document.createElement('div'); wrap.className='container'; document.body.appendChild(wrap);
    document.body.removeChild(wrap); // rebuild via mount
    mount();
  });
})();
